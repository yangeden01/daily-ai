...
- 照片與附件的本機新增、預覽、下載與移除
- `Daily.xlsx` 與舊版 `Record` 工作表的事件匯入、去重合併及覆蓋還原
...
- **舊版 Excel**：可直接讀取 `Record` 工作表；分類代碼 `P`、`C`、`CP` 會轉為「私事」、「公事」、「機密公事」。瀏覽器無法直接讀取 Excel 內指向電腦路徑的連結檔案，照片與附件請先轉成完整 ZIP 備份。
...
1. 開啟 **Settings**。
2. 日常資料交換可使用「匯出備份」建立 Excel；「合併匯入」會保留既有事件並略過重複資料，「覆蓋匯入」則會取代全部事件。
3. 舊版 `Record` Excel 也可選擇「合併匯入」，不必先手動改成 Daily.xlsx。
4. 要保留照片與附件時，請使用 ZIP；「合併完整備份」會補入缺少的事件與附件，「覆蓋完整還原」則會取代目前所有事件與附件。
5. 匯入前確認提示中的範圍，完成後再到 Daily Timeline 抽查資料。
