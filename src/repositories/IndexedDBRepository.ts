...
export class IndexedDBRepository implements EventRepository {
  ...
  private async triggerWidgetSync(): Promise<void> {
    try {
      ...
      await syncWidgetSnapshot(all)
    } catch {
      // Widget sync failure must never disrupt database operations
    }
  }
