...
describe('LocalQueryEngine', () => {
  ...
  it('finds events matching single character keywords such as 薪 or 稅', async () => {
    const { repository, engine } = await createEngine()
    await repository.replaceAll([
      ...(await repository.getAll()),
      createTestEvent({ id: 'salary-raise', date: '2026-06-15', title: '年中加薪通知', detail: '底薪調升 5%', category: '公事' }),
    ])
    const resultForSalary = await engine.query('薪', now)
    expect(resultForSalary?.events).toMatchObject([{ id: 'salary-raise' }])
    expect(resultForSalary?.count).toBe(1)

    const resultForTax = await engine.query('稅', now)
    expect(resultForTax?.count).toBe(1)
    expect(resultForTax?.events).toMatchObject([{ id: 'tax-2025' }])
  })
