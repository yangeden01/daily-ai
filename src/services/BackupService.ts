...
const eventFingerprint = (event: Event): string => JSON.stringify([
  ...
  event.calendarType ?? null,
  event.lunarMonth ?? null,
  event.lunarDay ?? null,
  event.isLeapMonth ?? false,
...
export class BackupService {
  ...
  async exportWorkbook(): Promise<Uint8Array> {
    ...
    events.forEach((event) => {
      eventsWorksheet.addRow({
        ...
        lunarMonth: event.lunarMonth ?? null,
        lunarDay: event.lunarDay ?? null,
        isLeapMonth: event.isLeapMonth ? true : false,
        targetYear: event.targetYear ?? null,
      })
  ...
  private readAndValidateEvents(workbook: ExcelJS.Workbook): Event[] {
    ...
    worksheet.eachRow((row, rowNumber) => {
      ...
      const rawRecordType = this.cellText(row, 11)
      const recordType = rawRecordType === 'note' ? 'note' : rawRecordType === 'anniversary' ? 'anniversary' : 'daily'

      if (!id || (recordType === 'daily' && !date) || !title || !detail || !category || !createdAt || !updatedAt) {
      ...
      const rawCalendarType = this.cellText(row, 14)
      const calendarType = rawCalendarType === 'lunar' ? 'lunar' : rawCalendarType === 'solar' ? 'solar' : undefined
      const lunarMonthVal = Number(this.cellText(row, 15)) || undefined
      const lunarDayVal = Number(this.cellText(row, 16)) || undefined
      const isLeapMonthVal = this.cellText(row, 17) === 'true' || row.getCell(17).value === true
      const targetYearVal = Number(this.cellText(row, 18)) || undefined

      events.push({
        id,
        date: date || (recordType === 'anniversary' ? '2000-01-01' : ''),
        title,
        detail,
        category,
        amount: parsedAmount,
        ...
        ...(recordType === 'anniversary' ? {
          ...
          ...(targetYearVal ? { targetYear: targetYearVal } : {}),
        } : {}),
      })
    })
